        ...
        <div class="row pr-3">
          <div id="preview" class="col pr-0">
            <div class="card bg-transparent mr-5 position-absolute">
              <img class="card-img-top" :src="`/${pics[i].md}`" alt="Card image cap" @mouseover="toggle" @mouseout="toggle">
              <div id="mask" class="position-absolute" :class="{'d-none':hide}" :style="maskStyle" style="z-index:100;background-color:lightYellow;opacity:.5;width:176px;height:176px"></div>
              <div id="super-mask" class="position-absolute" @mouseover="toggle" @mousemove="move" @mouseout="toggle"  style="width:392px;height:392px;z-index:110;" ></div>
              <div id="div-lg" class="position-absolute" :class="{'d-none':hide}" style="left:400px;width:402px;height:402px;z-index:100;" :style="{'background-image':`url(/${pics[i].lg})`,'background-position':`-${parseInt(maskStyle.left)*2}px -${parseInt(maskStyle.top)*2}px`}"></div>
              <div class="card-body p-0 text-center">
                 <img src="/img/product_detail/hover-prev.png" class="btn float-left btn-light border-0 p-1 pt-4 pb-4"  :class="{disabled:times==0}" @click="moveRight">
                <!-- 四张小图 -->
                <div style="width:248px; overflow:hidden" class="d-inline-block pt-2 mx-0 m-auto">
                  <!-- <ul class="list-unstyled mb-0"> -->
                  <ul class="list-unstyled mb-0" :style="{width:pics.length*62+'px',
                  'margin-left':-62*times+'px'}"  @mouseover="changei" >
                ...
                <img src="/img/product_detail/hover-next.png" class="btn float-right btn-light border-0 p-1 pt-4 pb-4" :class="{disabled:times==pics.length-4}" @click="moveLeft">
              </div>
            </div>
          </div>
           <div id="details" class="col pl-0">
            <h6 class="font-weight-bold" v-text="product.title">
            ...
            <div class="alert alert-secondary small" role="alert">
              <div>
                <span>学员售价：</span>
                <h2 class="d-inline text-primary font-weight-bold" v-text="`￥${product.price.toFixed(2)}`"></h2>
              </div>
              <div>
                <span>服务承诺：</span>
                <span v-text="product.promise">*退货补运费 *30天无忧退货 *48小时快速退款 *72小时发货</span>
              </div>
            </div> -->
            <!-- 客服 -->
            <p class="mb-1">
            ...
            <div>
              ...
              <div class="float-left w-75">
                <router-link v-for="(sp,i) of specs" :key="i" class="btn btn-sm btn-outline-secondary" :class="{active:sp.lid==lid}" :to="`/details/${sp.lid}`" v-text="sp.spec">双核i5/8GB内存/128GB闪存</router-link>
              </div>
...
export default {
  ...
  data(){
    return{
      // 就算ajax没回来 我也有默认值
      // 为什么一个对象两个数组
     //console.log(result.data);因为这里也是一个对象两个数组
      product:{price:0},
      specs:[],
       //因为页面上需要加载一组图片，所以data中需要有一个保存所有图片的数组
      pics:[
        // 要先用空字符串给个初始值，否则会报md is no dufeinder
        {md:"",lg:""}
      ],
      //因为小图片列表左右移动核心是点击的次数，所以data中需要用time变量记录点击了几次
      times:0,
      //因为中图片和大图片具体使用哪一张图片和选中的第几张的下标有直接关系
      //比如：鼠标进入ul下第2张小图片pian[1]
      //则中图片采用pics中[1]位置的图片的md版本
      //大图片采用pics中[1]位置的图片的lg版本 
       //比如：鼠标进入ul下第2张小图片pian[2]
      //则中图片采用pics中[2]位置的图片的md版本
      //大图片采用pics中[2]位置的图片的lg版本 
      //所以只需要一个变量i,就可以控制中图片和大图片
      i:0,
      //因为需要同时动态控制小遮罩层和大图片的显示隐藏，所以需要一个变量
      hide:true,
      // 因为要修改mask的位置，所以未mask绑定style对象
      maskStyle:{
       left:0,
       top:0
      }
  ...
  methods:{
    ...
    changei(e){
      //  事件委托
      // 以后载框架中增删改查都简化了唯独事件没有简化
      // e.target代替this  this.i=i;
      // 只允许img元素触发事件
    ...
    loadPage(){
     this.axios.get("/details",{
      params:{lid:this.lid}//vue第三天讲的axios
    }).then(result=>{
